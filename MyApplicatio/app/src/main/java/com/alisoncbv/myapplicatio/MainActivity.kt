...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        productos.setOnClickListener {
            val vistaProductos = Intent(this, Producto::class.java)
            startActivity(vistaProductos)

            val adminBD = AdminSQL(this, "AlisonBD", null, 1)
            val bd = adminBD.writableDatabase
            val consulta = bd.rawQuery("select codigo, nombre, precio from producto", null)

            if (consulta.moveToFirst()) {
                while(consulta.moveToNext()){
                    productos.setText(consulta.getString(0))
                    productos.setText(consulta.getString(1))
                    productos.setText(consulta.getString(2))
                }
            }else{
                Toast.makeText(this, "No hay procustos registrados", Toast.LENGTH_LONG).show()
            }
            bd.close()
        }
